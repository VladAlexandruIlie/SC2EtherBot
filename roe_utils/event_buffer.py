...
class EventBufferSQLProxy:
    ...
    def __init__(self, n, capacity, exp_id, actor_id, user='roe', password='RarityOfEvents', host='localhost',
                 database='roe', event_clip=0.01):
        self.n = n
        self.exp_id = exp_id
    ...
    def get_events(self):
        ...
        for i in range(self.n):
            if i > 0:
                rows += ", "
            rows += "Event{}".format(i)
        cmd = "SELECT " + rows + " FROM Event WHERE ExperimentID = {} AND ActorID != {} ORDER BY EventID DESC LIMIT {}".format(
            self.exp_id, self.actor_id, self.capacity)
        mycursor.execute(cmd)
